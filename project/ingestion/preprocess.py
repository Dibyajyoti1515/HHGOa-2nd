...
Ported from Cell 5 of Update_version_2.ipynb ("Preprocessing (cleanup
BEFORE chunking)"). The notebook's print() progress line is now routed
through the central logger instead -- no other logic changed.
"""
...
def preprocess_corpus(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info("Validity filter: %s -> %s rows", f"{before:,}", f"{len(df):,}")
    return df
